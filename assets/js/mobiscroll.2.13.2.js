        ...
        that.init = function (ss) {
            ...
            if (that._isInput) {
                $elm.on('change.dw', function () {
                    ...
                    var yearL =  $(".dwwl0 .dw-sel .dw-i").html();
                    var monthL = $(".dwwl1 .dw-sel .dw-i").html();
                    var dayL = $(".dwwl2 .dw-sel .dw-i").html().substr(0,4);
                    var weekL = $(".dwwl2 .dw-sel .dw-i .dw-day").html();
                    var hourL = $(".dwwl3 .dw-sel .dw-i").html().substr(0,2);
                    var minL = $(".dwwl4 .dw-sel .dw-i").html().substr(0,2);
                    if($elm.attr("id")=="tests"){
                        $(".startTime .timeCon div").html(yearL+"年"+monthL+dayL+"("+weekL+")");
                        $(".startTime .timeSpan").html(hourL+":"+minL);
                    }else if($elm.attr("id")=="test"){
                        $(".endTime .timeCon div").html(yearL+"年"+monthL+dayL+"("+weekL+")");
                        $(".endTime .timeSpan").html(hourL+":"+minL);
                    }
                });
            }
        };
        ...
        that._generateContent = function () {
            ...
            $.each(s.wheels, function (i, wg) { // Wheel groups
                ...
                $.each(wg, function (j, w) { // Wheels
                    ...
                    html += '<' + (hasFlex ? 'div' : 'td') + ' class="dwfl"' + '>' +
                                ...
                                if (regen) {
                                    $('.dw-li', t).each(function () {
                                        var that = $(this),
                                            ...
                                            str = dord.replace(/[my]/gi, '').replace(/dd/, '月'+(d < 10 ? '0' + d : d) + (s.daySuffix || '')).replace(/d/, d + (s.daySuffix || ''));
                                        $('.dw-i', that).html(str.match(/DD/) ? str.replace(/DD/, '日&nbsp;<span class="dw-day">' + s.dayNames[w] + '</span>') : str.replace(/D/, '<span class="dw-day">' + s.dayNamesShort[w] + '</span>'));
                                    });
                                }
                            }
                            if (minprop && mind) {
